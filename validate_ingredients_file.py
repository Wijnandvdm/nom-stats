...
def validate_ingredients_csv(csv_path: str) -> List[str]:
    ...
    for col in [
        ...
        if col in df.columns:
            bad_values = df[
                ~df[col].apply(
                    lambda x: str(x).replace(".", "", 1).isdigit() or pd.isna(x)
                )
            ]
    ...
    missing_units = df[
        df["measurement_unit"].isnull()
        | (df["measurement_unit"].astype(str).str.strip() == "")
    ]
    if not missing_units.empty:
        rows = [
            f"⚠️  Line number {i + 2} and ingredient '{row['name']}'"
            for i, row in missing_units.iterrows()
        ]
        issues.append(
            "".join(rows) + " is missing a 'measurement_unit'. Please specify one."
        )
    ...
    if not mismatches.empty:
        ...
        issues.append(
            "".join(rows)
            + f""" has 'measurement_unit' without 'weight_per_unit'.
            Please provide 'weight_per_unit' or use a base unit: {base_units}"""
        )
    if not issues:
